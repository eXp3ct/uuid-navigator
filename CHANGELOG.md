...
## [1.13.0] - 2026-08-17
### Fixed
- Настройка `showNotifications` (и другие, влияющие на уведомления в командах) читалась один раз при активации расширения и не подхватывала изменения без перезагрузки окна — теперь читается заново при каждом вызове команды

## [1.12.0] - 2026-08-17
### Added
- Поддержка конфигов классов/объектов в формате YAML (`*.class.yaml`, `*.object.yaml`) и JSON (`*.class.json`, `*.object.json`) — та же модель класс+свойства+объекты, что и в SQL-миграциях
- Fuzzy-матчинг классов по имени папки/алиасу (нормализация + расстояние Левенштейна) для привязки объектов и свойств, когда точное совпадение не найдено
  - Настройка `fuzzyClassMatching` — включение/выключение fuzzy-матчинга
  - Настройка `fuzzyClassMatchThreshold` — порог схожести (0-1)
- Резервная привязка свойств к классу по имени папки/алиасу (по аналогии с объектами), если для свойства нет явной связи в `classes_property_definitions`
### Fixed
- Критический баг парсинга SQL: значения с вложенными скобками внутри строк (например `jsonb`-настройки вида `two_pass_tsvector(jsonb_to_text({0}))`) обрывали разбор всех последующих строк INSERT — часть свойств/объектов терялась молча без ошибки
- Файлы в `bin/`/`obj/` (дубли сборки .NET) больше не попадают в область поиска и парсинга конфигов
### Changed
- Внутренний рефакторинг парсера: общий интерфейс `ConfigFileParser` (SQL/YAML/JSON) и единый `ConfigFileRepository` для поиска файлов вместо разрозненных вызовов `findFiles('**/*.sql')` по коду
